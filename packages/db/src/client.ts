...
// This package therefore needs no workers-types dependency; apps resolve
// `DurableObjectStorage` from their `worker-configuration.d.ts`.
...
// Row types selected from the schema, so consumers
// (sessionizer, MCP description) never hand-write a shape that drifts from it. `RawEvent`, not
// `Event`: the latter shadows the Workers / DOM global.
