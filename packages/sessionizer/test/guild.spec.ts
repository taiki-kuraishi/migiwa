...
describe("reduceGuildCreate", () => {
  ...
  // The reconciliation loop must scope "missing from the snapshot" to this guild's own open rows (spec §6.3: "この guild の open 行のうち、スナップショットに居ない user を close"), the same way reducePresenceStatus/reduceActivities/reduceVoice already scope their own lookups by (guild_id, user_id) instead of trusting the caller to pre-filter `open`.
