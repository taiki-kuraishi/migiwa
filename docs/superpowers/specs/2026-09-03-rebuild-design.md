...
`VoiceStateSlice` は `guild_id` と `self_stream` を optional にしている。`self_stream` が無い
payload は `false` として読む(rule "A required slice field has no fallback downstream" の対象外:
slice 側で optional と宣言し、ここにそう書いている)。
