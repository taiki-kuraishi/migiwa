...
bunx vp run oxlint --deny-warnings && bunx vp run oxfmt --check && bunx vp run type-check && bunx vp run -r type-check && bunx vp run -r test && bun run knip && bun dedupe --check
```
